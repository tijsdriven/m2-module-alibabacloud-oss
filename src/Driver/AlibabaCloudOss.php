...
class AlibabaCloudOss implements \Magento\RemoteStorage\Driver\RemoteDriverInterface
{
    ...
    private function readPath(string $path, $isRecursive = false): array
    {
        ...
        $itemsList = [];
        $contents = $this->adapter->listContents($this->fixPath($relativePath), $isRecursive);
        foreach ($contents as $item) {
            ...
            if (!empty($path)
                ...
                //phpcs:ignore Magento2.Functions.DiscouragedFunction
                $itemsList[] = rtrim($this->getAbsolutePath(dirname($path), $path), '/');
            }
        }
